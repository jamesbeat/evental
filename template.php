...
drupal_add_js($js_base.'/jquery.scrollTo.js', array(
  'type' => 'file',
  'scope' => 'footer',
  'group' => JS_THEME,
  'weight' => 25,
...
function evental_preprocess_page(&$vars, $hook) {
...
function evental_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from evental_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
  ...
  if(theme_get_setting('evental_lazy_enabled') == TRUE){
	  ...
	  $themedir = drupal_get_path('theme', 'evental');
	  $regex = '#<img([^>]*) src="([^"/]*/?[^".]*\.[^"]*)"([^>]*)>((?!</a>))#';
...
function evental_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function evental_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('evental_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('evental_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('evental_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('evental_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('evental_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function evental_id_safe($string) {
 ...
 *
 */
function evental_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = evental_id_safe($element['#title']);
 ...
 */
function evental_preprocess_menu_local_task(&$variables) {
 ...
 */
function evental_menu_local_tasks(&$variables) {
  $output = '';
